...
struct traversable_graph : private graph {
  ...
  auto edgeMapData(VS& vs, F& f, long threshold = -1, const flags& fl = 0) {
    ...
    if (vs.isDense && vs.size() > (n / 10)) {
      // TODO: edgeMapDenseForward
      std::cout << "EdgeMapDense (shortcut) " << vs.size() << std::endl;
  ...
  auto edgeMapData(VS& vs, F& f, parlay::sequence<edge_node*>& flat_snap,
    ...
    if (vs.size() + out_degrees > static_cast<size_t>(threshold) &&
        !(fl & no_dense)) {
      std::cout << "EdgeMapDense" << " " << vs.size() << std::endl;
      return edgeMapDense<F>(vs, flat_snap, f, fl);
    }
  ...
  void insert_edges_batch_inplace(size_t m, Edge* edges, Replace&
      replace, bool sorted = false, bool remove_dups = false, size_t
      nn = std::numeric_limits<size_t>::max(), bool run_seq = false) {
    G::insert_edges_batch_3(m, edges, replace, sorted, remove_dups, nn, run_seq);
  }

  template <class Edge>
  void insert_edges_batch_inplace(size_t m, Edge* edges, bool sorted =
      false, bool remove_dups = false, size_t nn =
      std::numeric_limits<size_t>::max(), bool run_seq = false) {
  ...
  void delete_vertices_inplace(size_t m, VertexId* vertices, bool sorted = false,
                          bool remove_dups = false) {
    G::delete_vertices_inplace(m, vertices, sorted, remove_dups);
  }
