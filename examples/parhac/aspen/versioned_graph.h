...
struct versioned_graph {
  ...
  versioned_graph(snapshot_graph&& G) : current_timestamp(0) {
    ...
    std::cout << "After creating VG, ref_cnt = " << G.ref_cnt() << " root = " << G.get_root() << " timestamp = " << latest_timestamp() << std::endl;
    G.clear_root();
  }
        ...
        while(true) {
          ...
          if (ref_ct_before > 0) {
            if (cpam::utils::atomic_compare_and_swap(&std::get<0>(std::get<1>(table_ref)), refct_and_ts, next_value)) {
              return version(ts, &table_ref, std::move(snapshot_graph(std::get<1>(std::get<1>(table_ref)))));
            }
          } else { // refct == 0
            break;
          }
...
//            std::cout << "Decremented ref-ct on root: " << root << std::endl;
 ...
 //   std::cout << "S ref = " << S.graph.ref_cnt() << " root = " << S.graph.get_root() << std::endl;
...
//    std::cout << "Next root (insert) = " << G_next.get_root() << " ref_cnt = " << Tree::ref_cnt(G_next.get_root()) << std::endl;
