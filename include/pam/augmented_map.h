...
public:
  ...
  template <class Func>
  void iterate_aug(const Func& f) {
    Tree::iterate_aug(root, f);
  }
  ...
  template <class F>
  static void foreach_index(const M& m, F f, size_t start=0,
			    size_t granularity = utils::node_limit) {
    Map::foreach_index(m, f, start, granularity); }
  template <class F>
  static void foreach_index_2(const M& m, F f, size_t start=0,
			    size_t granularity = utils::node_limit) {
    Map::foreach_index(m, f, start, granularity); }
public:
