...
/* ============================ Compressed Join =============================== */
  ...
  static node* right_join(node* t1_o, node* t2, regular_node* k) {
    ...
    if (Node::is_left_heavy(t->rc, t->lc)) {
      ...
      if (Node::is_single_rotation(Node::cast_to_regular(t->rc), 0)) {
        ...
        // TODO(7/21/21): check condition below
        assert(Node::is_regular(t->rc));
  ...
  static node* balanced_join(node* l, node* r, regular_node* e) {
    ...
    // TODO (7/21/21)
    // if (!Node::is_regular(l) || !Node::is_regular(r)) {
...
//    std::cout << "t1_reg = " << t1_reg << " t2_reg = " << t2_reg << " t1_size = " << Node::size(t1) << " t2_size = " << Node::size(t2) << std::endl;
...
//        size_t ret_size = Node::size(ret);
    ...
    if (balanced || !Node::could_single_rotate(t1, t2)) {  // TODO: need to check this in other spots?
