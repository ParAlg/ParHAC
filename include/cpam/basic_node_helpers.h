...
static typename Node::node* two_compressed_nodes(typename Node::ET* stack, size_t tot,
  ...
  template <class Node>
  static typename Node::node* four_compressed_nodes(typename Node::ET* stack, size_t tot, size_t B,
                                     typename Node::regular_node* e = nullptr) {
...
*/
template <class Node>
static typename Node::node* make_compressed(typename Node::ET* stack, size_t tot, size_t B) {
  assert(tot >= B);
  assert(tot <= 8*B+2);
  using ET = typename Node::ET;
...
template <class Node>
static typename Node::node* make_compressed(typename Node::node* l, typename Node::node* r, typename Node::regular_node* e, size_t B) {
...
// diff-encoding.
template <class Node>
static typename Node::node* make_compressed_node(typename Node::node* b, size_t B) {
  using ET = typename Node::ET;
  ET stack[2*B];
  size_t offset = 0;
